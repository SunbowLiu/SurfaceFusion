...
def drawweight(args, svd_list):
    ...
    plt.plot(svd_list[2], linewidth='5', label='Trivial')
    plt.plot(svd_list[0], linewidth='5', label='Random')
    plt.plot(svd_list[1], linewidth='5', label='Important')
    ...
    if args.path:
        plt.savefig(args.path, format="png", bbox_inches='tight')
    else:
        plt.savefig(args.input + '.bestworse.svd.png', format="png", bbox_inches='tight', dpi=300)
...
def getweight(args, input):
    ...
    if args.feature == 'fgta':
        ...
        mean_weight = state['model']['decoder.ta_weight'][-1].float()
    ...
    ori = emb[:, np.random.permutation(mean_weight)]
    # now=emb[:,soft_weight]
